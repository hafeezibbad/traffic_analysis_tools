...
def load_manuf_file(manuf_file_path: str) -> Optional[dict]:
    ...
    manufs = dict()
    try:
        logging.debug('Looking for wireshark manuf file at `%s`', manuf_file_path)
        with open(manuf_file_path, 'r') as f:
            logging.info('Reading manuf file from `%s`', manuf_file_path)
            content = f.readlines()
            for line in content:
                if line and not line.startswith('#'):
                    entry = line.rstrip().split('\t')
                    manufs[entry[0]] = entry[1:]
        logging.info('`%s` entries retrieved.', len(manufs.keys()))
        return manufs
    except FileNotFoundError:
        logging.error('File not found at `%s`', manuf_file_path)

    except Exception as e:
        logging.error('Unable to process file: `%s`. Error: `%s`', manuf_file_path, e)
...
def fetch_manuf_file(manuf_file_uri: str = WIRESHARK_MANUF_FILE, manuf_file_path: str = '') -> bool:
    ...
    try:
        ...
        logging.info('MANUFs file successfully retrieved from `%s` and stored at `%s`', manuf_file_uri, manuf_file_path)

        return True
    except HTTPError as e:
        logging.error('Server failure in fetching resource from `%s`. Error code: `%s`', manuf_file_uri, e.code)

    except URLError as e:
        logging.error('Unable to retrieve the resource from `%s`. Reason `%s`', manuf_file_uri, e.reason)

    except IOError as e:
        logging.error('Unable to store manuf file at `%s`. Error `%s`', manuf_file_uri, e)

    except Exception as e:
        logging.error('Unable to retrieve and store manuf file. Error `%s`', e)
